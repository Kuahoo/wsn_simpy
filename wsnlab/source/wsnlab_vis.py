...
class Node(wsnlab.Node):
    ...
    def set_label(self, label):
        """Updates the text label of the node in the visualization.

        Args:
            label (str): Text to display on the node.
        """
        if hasattr(self.scene, "nodelabel"):
            self.scene.nodelabel(self.id, label)
...
class Simulator(wsnlab.Simulator):
    ...
    def __init__(
        ...
        if self.visual:
            ...
            self.scene.linestyle(
                "wsnsimpy:unicast", color=(0, 0, 1), width=3, arrow="head"
            )
            self.scene.linestyle("wsnsimpy:collision", color=(1, 0, 0), width=3)
            self.scene.linestyle("parent", color=(0, 0.8, 0), arrow="tail", width=2)
            if title is None:
                title = "WsnSimPy"
